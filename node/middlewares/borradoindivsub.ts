...
export async function borradoindivsub(ctx: Context, next: () => Promise<any>) {
    const {
        clients: {
            // apiVTEX : ApiVTEX
            borradoindivsub : Borradoindivsub
        }
    } = ctx
    ...
    // for await (const skuId of objetoDiv) {
        let responseSpecSku: response = await Borradoindivsub.deleteindivsub(arraySkuId, headersRequest)
