...
export async function agrupamiento(ctx: Context, next: () => Promise<any>) {
    const {
        clients: {
            // apiVTEX : ApiVTEX
            entidadeslegales : EntidadesLegales
        }
    } = ctx
    ...
    console.log("el arrayskuidagrup",arraySkuId)
    ...
    // for (let i=0; i<objeto.length; i++){
        let i = 0
    for await (const skuId of objetoDiv) {
        ...
        console.log("el responseSpecSku data2:", responseSpecSku.data);
        ...
        if (responseSpecSku.data.length >0 ){
            Object.assign(objeto[i], {esquema:"entidad"}, {tipotransaccion: responseSpecSku.data[0].tipotransaccion});
        }
        if (responseSpecSku.data.length === 0){
                Object.assign(objeto[i], {esquema:"market"}, {tipotransaccion:"venta"});
        }
